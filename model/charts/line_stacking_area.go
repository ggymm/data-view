...
var LineStackingAreaGetDataHandle = &ChartDataHandler{RunGetDataFromDB: func(db *sql.DB, chartDataParams *schema.ChartDataParams) (string, error) {
	...
	dataResults, err := LineStackingAreaFormatRows(rows, chartDataParams)
	if err != nil {
		return EmptyString, err
	}
...
func LineStackingAreaFormatRows(rows *sql.Rows, chartDataParams *schema.ChartDataParams) (*map[string]interface{}, error) {
